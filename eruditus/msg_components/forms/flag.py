...
class FlagHoardingForm(discord.ui.Modal, title="Flag hoarding form"):
    flag = discord.ui.TextInput(
        label="Flag",
        placeholder=r"ctf{s0m3th1ng_l33t}",
    )
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        challenge["solve_time"] = int(datetime.now().timestamp())
        challenge["flag"] = self.flag.value
        challenge["hoarded"] = True

        solves_channel = interaction.client.get_channel(
            ctf["guild_channels"]["solves"])
        solvers = await parse_challenge_solvers(interaction, challenge, members)

        await interaction.followup.send("✅ Flag has been hoarded!")
        embed = discord.Embed(
            title="🎉 Challenge solved!",
            description=(
                f"**{', '.join(solvers)}** just solved "
